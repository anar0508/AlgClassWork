...
let filteredCopy= copy.filter((employee)=> {return employee.role==='Дизайнер'}); 
let totalWorkExp = filteredCopy
.map((employee)=>{ return employee.workExperience})
.reduce((prevRes, currRes)=>{return prevRes+currRes;})
console.log( totalWorkExp/filteredCopy.length);